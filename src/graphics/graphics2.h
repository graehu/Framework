...
namespace fw
{
   ...
   typedef blob::asset<Vertex> VertexBuffer;
   typedef blob::asset<uint32_t> IndexBuffer;
   typedef blob::asset<unsigned int> ImageBuffer;
...
namespace graphics2
{
   int init(void);// = 0; //initializing gl and what not for whatever window system is implemented
   int render(void);// = 0; //this'll have flipping the buffers
   bool register_shader(fw::hash::string/*name*/, const char* /*path*/, fw::shader::type /*type*/, const char* /*entry_point*/);// { return 0; } // being lazy here, don't want to have to implement this for everything
   bool register_material(fw::Material /*material*/);// { return 0; } // being lazy here, don't want to have to implement this for everything
   bool register_pass(fw::hash::string);// { return 0; }
   void set_shademode(int /*shademode*/); //{} // lazy
   void reset();// {}// lazy
   int shutdown(void);// = 0; //shutsdown the graphics engine
   int update(void);// = 0; //this is currently useless.
   //
   void render(camera* _camera);
   void render(physics::collider::polygon* _poly);	 
